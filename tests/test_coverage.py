...
def test_missing_labels_with_coverage_counts_vote_only_labels():
    track = TrackState(track_id=3)
    track.record_vote("EILEEN", 1.2)

    missing = missing_labels_with_coverage(
        tracks=[track],
        segments=[],
        known_labels={"EILEEN", "KYLE"},
    )

    assert missing == {"EILEEN"}
